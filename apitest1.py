...
#将excel表中的数据，遍历放进list
        ...
        for i in list1:
            ...
            if 'http://apitest.lyancafe.com/third/v1/order/check' == i['url']:
                for name in names:
                    print u'========================now running apitest of %s' %(name) ,',the %sst case' %n,'======================'
                    #print noncenonce_ex = 'nonce='+str(time.time()).split(='.')[0]+'000'
                    sign_ex = 'address='+ str(i['address']) + '&' +  'appid='+names[name][0]+ '&' + 'city=' + str(i['city']) + '&' + 'expectedid=' + str(i['expectedid'])+'&'+ nonce_ex + '&' + 'appkey='+ names[name][1]
                    #print sign_ex
                    sign = hashlib.sha1(sign_ex).hexdigest()
                    #print sign
                    getparams = {'appid':names[name][0],'nonce':nonce,'address':str(i['address']),'sign':sign,'city':str(i['city']),'expectedid':str(i['expectedid'])}
                    #print getparams
                    postparams = ''
                    putparams = ''
                    response = self.apicall(method,url,getparams,postparams,putparams,headers)
    ...
    def calculate_sign_params(self,file1,names=None):
        '''计算不同接口的sign签名'''
        response = {}
        names = names or {}
        sign_list = []
        ...
        list1 = self.excel_table_byindex(file1)
        for i in list1:
            methodlist.append(i['method'])
            urllist.append(str(i['url']))
        #print sign_list
        #print params_list
        #print urllist,methodlist
        #return sign_list,params_list
        return params_list,methodlist,urllist
    def response_compare(self,file1,names):
        headers = {'content-type': 'application/x-www-form-urlencoded'}
        params_list,methodlist,urllist = self.calculate_sign_params(file1,names)
        n = 1
        for i in zip(params_list,methodlist,urllist):
            method = i[1]
            url = i[2]
            if method == 'GET':
                getparams = i[0]
...
if __name__ =='__main__':
    ...
    a = CallApi()
        ...
        #input("Press <enter>")
    #if 'the_third' in api_case:
        #a.calculate_sign_params(api_case,names)
        #input('press <enter>')
    if 'the_third' in api_case:
        a.response_compare(api_case,names)
        #input("Prease <enter>")
    #if 'the_third' in api_case:
        #a.response_compare(api_case)
        #input("Prease <enter>")
